...
class PrefixTree:
    ...
    def complete(self, prefix):
        ...
        # Creates a list of completions in prefix tree, an empty array to hold all completed strings
        completions = []
        #checks prefix is equal to nothing
        if prefix == '':
            # Return null
            return self.strings()
        # use the _find function to find the prefix within the string
        node = self._find_node(prefix)
        #checks  if the value at the start is not equal to nothing
        if node[0].character != '':
...
def main():
    #  test case of string with partial substring overlaps
